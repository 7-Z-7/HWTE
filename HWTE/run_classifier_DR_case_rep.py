...
import csv
import os
import modeling_DR_rep as modeling
import optimization
import tokenization
...
def create_model(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels, num_labels,sin_position):
  ...
  # position_attention_probs=model.get_all_attenrion_probs()
  position_attention_probs=model.get_attenrion_probs(0)
  position_attention_probs=tf.reshape(position_attention_probs,[batch_size,day_length,-1,route_length,route_length])
  ...
  Dmodel=modeling.DRertModel_new(
      ...
      sin_position=sin_position)
  day_attention_probs = Dmodel.get_attenrion_probs(0)

  output_layer = Dmodel.get_pooled_output()

  representation=output_layer
  ...
  with tf.variable_scope("loss"):
    ...
    return (loss, per_example_loss, logits, probabilities,representation,position_attention_probs,day_attention_probs)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, logits, probabilities,representation,position_attention_probs,day_attention_probs) = create_model(
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          ...
          predictions={"probabilities": probabilities,
                       ...
                       "rep":representation,
                       "position_att":position_attention_probs,
                       "day_att":day_attention_probs},
          scaffold_fn=scaffold_fn)
    return output_spec
...
def main(_):
  ...
  do_eval_train_set=FLAGS.do_train
  ...
  if FLAGS.do_predict:
    ...
    predict_examples = processor.get_dev_examples(FLAGS.data_dir,rate=FLAGS.data_sampling_rate,rng=rng)
    ...
    # output_predict_file = os.path.join(FLAGS.output_dir, "test_results.pickle")
    # with open(output_predict_file, "wb") as f:
    #     tf.logging.info("***** Predict results *****")
    #     pickle.dump(result,f)

    # output_predict_file = os.path.join(FLAGS.output_dir, "test_results.tsv")
    output_predict_file = os.path.join(FLAGS.output_dir, "test_rep_att.npy")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        reps = prediction["rep"]
        position_att = prediction["position_att"]
        day_att = prediction["day_att"]
        all_list.append([probabilities,labels,inputs,durings,masks,reps,position_att,day_att])
        # probabilities_list.append(probabilities)
        if i >= num_actual_predict_examples:
          break
        # output_line1 = str(probabilities) + "*"
        # # output_line2 = ",".join(
        # #     str(label)
        # #     for label in labels) + "\n"
        # output_line2 = str(labels) + "\n"
        # output_line3 = ",".join(
        #     str(input)
        #     for input in (inputs)) + "\n"
        # output_line4 = ",".join(
        ...
        #     str(mask)
        #     for mask in masks) + "\n\n"
        # writer.write(output_line1)
        # writer.write(output_line2)
        # writer.write(output_line3)
        # writer.write(output_line4)
        # writer.write(output_line5)
        # num_written_lines += 1
      # numpy.save(writer,probabilities_list)
      numpy.save(writer,all_list)
