...
def create_downstream_instances_DR(input_files,input_label,input_promotion,input_EMP_INFO,tokenizer, max_seq_length,
  ...
  user_info_dict=info_load(input_EMP_INFO=input_EMP_INFO,load_type=3,pickle_path='user_info_dict.pickle')
    ...
    if user_name in uuap2id:
      if uuap2id[user_name] in user_label_dict:
        instances[uuap2id[user_name]]=create_ds_instances_from_DR(
                all_WR, user_label_dict,user_info_dict,uuap2id,user_name,user_index, max_seq_length, short_seq_prob,
      ...
      if one[0]>=start_time:
        ...
        for i in range(1,a_day):#day in one instances
          if label_day-i*24*60*60 in WR:
            ...
            text_a_list.append([x[0] for x in one_day_pair])
            text_a_during_list.append([x[1] for x in one_day_pair])
            # one_day_info = [user_Id, int(day_list[WR_index]), one_day_mainplace]

        instances.append([guid,text_a_list,text_a_during_list,label,info_label,schedule_list,main_place_list,day_position])#[guid,text_a,text_a_during,label]
  return instances
...
def main(_):
  ...
  instances = create_downstream_instances_DR(
    input_files,input_label,input_promotion,input_EMP_INFO, tokenizer, FLAGS.max_seq_length, FLAGS.dupe_factor,
    FLAGS.short_seq_prob, FLAGS.masked_lm_prob, FLAGS.max_predictions_per_seq,
    rng)
  ...
  path = os.path.join(output_files[0], 'all_dr_ds_instances2.pickle')
