...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu, stop_gradient=False,stop_variable_names=None):
  ...
  if stop_gradient:
    for i,var in enumerate(tvars):
      if var.name in stop_variable_names:
          del(tvars[i])
...
def create_ag_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu, gradient_accumulation_multiplier=1,stop_gradient=False,stop_variable_names=None):
  """Creates an optimizer training op."""
  global_step = tf.train.get_or_create_global_step()
  ...
  if gradient_accumulation_multiplier>1:
  ...
  ############################
    ...
    # This is how the model was pre-trained.(old gradients)
    (grads, _) = tf.clip_by_global_norm(grads, clip_norm=1.0)
