...
level3base_root_path='../../data/case_data/level3base/'
...
def rep_plot(path):
    ...
    print(len(data))
    print(data[0].keys())
    reps=[one['rep'] for one in data]
    labels=[one['label'] for one in data]
    results=[one['result'] for one in data]
    # 标准化
    scaler = StandardScaler()
    scaler.fit(reps)
    scaled = scaler.transform(reps)

    # PCA
    randomized_pca = PCA(n_components=2, svd_solver='randomized')
    pc = randomized_pca.fit_transform(scaled)
    ...
    for i,one in enumerate(results):
        ...
        # print(results[i])
        if one==labels[i]:
            new_pc_2.append(pc[i])
            new_label_2.append(labels[i])
            new_result_2.append(results[i])
        else:
            new_pc_1.append(pc[i])
            new_label_1.append(labels[i])
            new_result_1.append(results[i])
    pc_back=np.array(new_pc_1)
    pc_front=np.array(new_pc_2)
    print(len(pc_back))
    print(len(pc_front))
    pc=np.array(new_pc_1+new_pc_2)
    labels=new_label_1+new_label_2
    results=new_result_1+new_result_2
    ...
    for one in labels:
        if one==0:
            colour.append('#ff2121')
        elif one==1:
            colour.append('#2176ff')
        else:
            # colour.append('#FFC125')
            colour.append('#9ACD32')
    # plt.scatter(pc1, pc2, c=colour, edgecolors='#000000')
    plt.scatter(pc2, pc1, s=100 ,c=colour, edgecolors='#000000',alpha=0.6)
    # plt.scatter(pc2, pc1, c=colour,alpha=0.6)
    plt.yticks(size=12)
    plt.xticks(size=12)
    plt.show()
...
if __name__=='__main__':
    ...
    # plot_all(plot_path)
    all_rep_data_path=os.path.join(level3base_root_path,'all_rep.pickle')
    att_data_path=os.path.join(level3base_root_path,'385_att.pickle')
    rep_plot(all_rep_data_path)
