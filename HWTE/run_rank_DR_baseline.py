...
class NextProcessorBase(DataProcessor):
...
def convert_single_example(ex_index, example, max_seq_length,max_day_length,
  ...
  if len(label_list)>predict_max_place:
      label_list=label_list[:predict_max_place]
  ...
  while len(new_label_list)<predict_max_place:
  ...
  if ex_index < 5:
    ...
    tf.logging.info("label_list: %s " % " ".join([str(x) for x in example.label_list]))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["label_ids_list"] = create_int_feature(feature.label_ids_list)
...
def file_based_input_fn_builder_ori(input_file, seq_length,day_length, is_training,
                                drop_remainder,predict_max_place):
  ...
  name_to_features = {
      ...
      "label_ids_list": tf.FixedLenFeature([predict_max_place], tf.int64),
...
def create_model(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels_list, label_mask,sin_position,vocab_size):
  ...
  with tf.variable_scope("loss"):
    ...
    predict_emb = tf.nn.bias_add(predict_emb, output_bias)
    ...
    return (loss, all_example_loss, predict_score, probabilities)



def create_Bert(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels_list, label_mask,sin_position,vocab_size):
        # bert_config, is_training, input_ids, input_mask, segment_ids,
        #          labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
...
def create_Longformer(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels_list, label_mask,sin_position,vocab_size):
        # bert_config, is_training, input_ids, input_mask, segment_ids,
        #          labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
...
def create_Longformer_slid(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels_list, label_mask,sin_position,vocab_size):
        # bert_config, is_training, input_ids, input_mask, segment_ids,
        #          labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
...
def create_Longformer_go(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels_list, label_mask,sin_position,vocab_size):
        # bert_config, is_training, input_ids, input_mask, segment_ids,
        #          labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
...
def create_Performer(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels_list, label_mask,sin_position,vocab_size):
        # bert_config, is_training, input_ids, input_mask, segment_ids,
        #          labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
...
def create_LSTM(bert_config, is_training, input_ids_list, input_mask_list, during_list, position, labels_list, label_mask,sin_position,vocab_size):
        # bert_config, is_training, input_ids, input_mask, segment_ids,
        #          labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
...
def model_fn_builder_ori(bert_config, predict_max_place, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    label_ids_list=features["label_ids_list"]
    ...
    (total_loss, all_example_loss, logits, probabilities) = create_model(
        bert_config, is_training, input_ids_list, input_mask_list, during_list, position, label_ids_list,
        label_mask_list,sin_position,vocab_size)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(loss, label_ids_list, logits, predict_max_place):
      ...
      eval_metrics = (metric_fn,
                      [total_loss, label_ids_list, logits, predict_max_place])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
...
def Bert_fn_builder(bert_config, predict_max_place, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings,sin_position,stop_gradient,gradient_accumulation_multiplier,vocab_size,max_length_of_one):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    begin = [0, 0]
    size = [-1, max_length_of_one]
    input_ids_list = tf.slice(input_ids_list, begin, size)
    input_mask_list = tf.slice(input_mask_list, begin, size)
    during_list = tf.slice(during_list, begin, size)
    # input_ids_list=tf.reshape(input_ids_list,list_shape[0])
    # input_mask_list=tf.reshape(input_mask_list,list_shape[0])
    # during_list=tf.reshape(during_list,list_shape[0])
    # input_ids = features["input_ids"]
    ...
    (total_loss, all_example_loss, logits, probabilities) = create_Bert(
        bert_config, is_training, input_ids_list, input_mask_list, during_list, position, label_ids_list,
        label_mask_list,sin_position,vocab_size)
...
def Longformer_fn_builder(bert_config, predict_max_place, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings,sin_position,stop_gradient,gradient_accumulation_multiplier,vocab_size,max_length_of_one):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, all_example_loss, logits, probabilities) = create_Longformer(
        bert_config, is_training, input_ids_list, input_mask_list, during_list, position, label_ids_list,
        label_mask_list,sin_position,vocab_size)
...
def Longformer_slid_fn_builder(bert_config, predict_max_place, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings,sin_position,stop_gradient,gradient_accumulation_multiplier,vocab_size,max_length_of_one):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, all_example_loss, logits, probabilities) = create_Longformer_slid(
        bert_config, is_training, input_ids_list, input_mask_list, during_list, position, label_ids_list,
        label_mask_list,sin_position,vocab_size)
...
def Longformer_go_fn_builder(bert_config, predict_max_place, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings,sin_position,stop_gradient,gradient_accumulation_multiplier,vocab_size,max_length_of_one):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, all_example_loss, logits, probabilities) = create_Longformer_go(
        bert_config, is_training, input_ids_list, input_mask_list, during_list, position, label_ids_list,
        label_mask_list,sin_position,vocab_size)
...
def Performer_fn_builder(bert_config, predict_max_place, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings,sin_position,stop_gradient,gradient_accumulation_multiplier,vocab_size,max_length_of_one):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, all_example_loss, logits, probabilities) = create_Performer(
        bert_config, is_training, input_ids_list, input_mask_list, during_list, position, label_ids_list,
        label_mask_list,sin_position,vocab_size)
...
def LSTM_fn_builder(bert_config, predict_max_place, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings,sin_position,stop_gradient,gradient_accumulation_multiplier,vocab_size,max_length_of_one):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, all_example_loss, logits, probabilities) = create_LSTM(
        bert_config, is_training, input_ids_list, input_mask_list, during_list, position, label_ids_list,
        label_mask_list,sin_position,vocab_size)
...
def main(_):
  ...
  predict_max_place=FLAGS.predict_max_place
  ...
  model_fn = model_fn_builder(
      ...
      predict_max_place=predict_max_place,
      ...
      max_length_of_one=FLAGS.max_length_of_one
  )
  ...
  if FLAGS.do_train:
    ...
    file_based_convert_examples_to_features(
        train_examples, FLAGS.max_seq_length,FLAGS.max_day_length, tokenizer, train_file,predict_max_place)
  ...
  if do_eval_train_set:
    ...
    eval_input_fn = file_based_input_fn_builder(
        ...
        day_length=FLAGS.max_day_length,
        is_training=False,
        drop_remainder=eval_drop_remainder,predict_max_place=predict_max_place)
    ...
    with tf.gfile.GFile(output_eval_file, "w") as writer:
      ...
      for key in sorted(result.keys()):
        if 'accuracy' in key:
            tf.logging.info("  %s = %s", key, str(predict_max_place*result[key]))
            writer.write("%s = %s\n" % (key, str(predict_max_place*result[key])))
        else:
  ...
  if FLAGS.do_eval:
    ...
    file_based_convert_examples_to_features(
        eval_examples, FLAGS.max_seq_length,FLAGS.max_day_length, tokenizer, eval_file,predict_max_place)
  ...
  if FLAGS.do_predict:
    ...
    file_based_convert_examples_to_features(predict_examples,
                                            ...
                                            predict_file,predict_max_place)
    ...
    predict_input_fn = file_based_input_fn_builder(
        ...
        day_length=FLAGS.max_day_length,
        is_training=False,
        drop_remainder=predict_drop_remainder,predict_max_place=predict_max_place)
