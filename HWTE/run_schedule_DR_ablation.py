...
import csv
import os
import modeling_DR_ablation as modeling
import optimization
import tokenization
...
def create_model_SP(bert_config, is_training, input_ids, input_mask, durings, regress_list, sin_position,vocab_size):
        # bert_config, is_training, input_ids, input_mask, segment_ids,
        #          labels, num_labels, use_one_hot_embeddings):
  """Creates a classification model."""
...
def Drert_SP_fn_builder(bert_config, regress_num, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings,sin_position,stop_gradient,gradient_accumulation_multiplier,vocab_size):
  """Returns `model_fn` closure for TPUEstimator."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    # (total_loss, all_example_loss, logits, probabilities) = create_model(
    (total_loss, predict_percent) = create_model_SP(
        bert_config, is_training, input_ids, input_mask, durings, regress_list,sin_position,vocab_size)
...
def main(_):
  ...
  model_fn_builders = {
      'Drert-SP': Drert_SP_fn_builder,
      'Drert-PQ': model_fn_builder_ori,
      'Drert-PQ+NSP': model_fn_builder_ori,
      # 'Drert-R': Drert_R_fn_builder,
      # 'LSTM':LSTM_fn_builder,
      # 'CNN':CNN_fn_builder,
  }
  file_based_input_fn_builders = {
      'Drert-SP': file_based_input_fn_builder_ori,
      'Drert-PQ': file_based_input_fn_builder_ori,
      'Drert-PQ+NSP': file_based_input_fn_builder_ori,
      # 'Drert-R': file_based_input_fn_builder_ori,
