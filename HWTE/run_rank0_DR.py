...
class LeaveProcessorBase(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    return self._create_examples(path, "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    path = os.path.join(data_dir, 'eval.pickle')
    # path = os.path.join(data_dir, 'train.pickle')
    return self._create_examples(path, "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    path = os.path.join(data_dir, 'test.pickle')
    return self._create_examples(path, "test")

  def get_labels(self):
    """See base class."""
    return ["0", "1"]#0未离职，1离职

  def _create_examples(self, pickle_path, set_type):
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings,sin_position,stop_gradient):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      train_op = optimization.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu,stop_gradient,initialized_variable_names)
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        # tf.metrics.recall(label_ids,predictions)
