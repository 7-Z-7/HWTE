...
def model_fn_builder(bert_config, predict_max_place, init_checkpoint, learning_rate,
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    each_time=(end_time-start_time)/len(eval_examples)

    output_eval_file = os.path.join(FLAGS.output_dir, "eval_results.txt")
    with tf.gfile.GFile(output_eval_file, "w") as writer:
      tf.logging.info("***** Eval results *****")
      tf.logging.info("each_time= %s", str(each_time))
      writer.write("each_time= %s\n" % (str(each_time)))
      for key in sorted(result.keys()):
        if 'accuracy' in key:
            tf.logging.info("  %s = %s", key, str(predict_max_place*result[key]))
            writer.write("%s = %s\n" % (key, str(predict_max_place*result[key])))
