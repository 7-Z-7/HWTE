...
class RertModel(object):
  ...
  def get_all_attenrion_probs(self):
    return self.all_attention_probs

  def get_attenrion_probs(self,layer=-1):
    return self.all_attention_probs[layer]
...
def attention_layer(from_tensor,
  ...
  attention_probs_0 = tf.nn.softmax(attention_scores)

  # This is actually dropping out entire tokens to attend to, which might
  # seem a bit unusual, but is taken from the original Transformer paper.
  attention_probs = dropout(attention_probs_0, attention_probs_dropout_prob)
...
def transformer_model(input_tensor,
  ...
  if do_return_all_layers:
    ...
    return final_output,all_attention_probs
