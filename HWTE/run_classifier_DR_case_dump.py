...
def group(features):
    # update_op=tf.add_to_collection('feature',features)
    sample_group=tf.get_collection('feature')
    return sample_group,sample_group
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn_ori(per_example_loss, label_ids, logits, is_real_example,probabilities):
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example,probabilities,features):
        predictions = tf.argmax(logits, axis=-1, output_type=tf.int32)
        if num_labels==2:
            if binary_threshold>0:
                balance0=0.5/(1-binary_threshold)
        ...
        tf.add_to_collection('feature',features)
        sample_group=group(features)

        # sample_group=tf.metrics.accuracy(
        #     labels=label_ids, predictions=predictions, weights=is_real_example)
        accuracy = tf.metrics.accuracy(
            labels=label_ids, predictions=predictions, weights=is_real_example)
      ...
      eval_metrics = (metric_fn,
                      [per_example_loss, label_ids, logits, is_real_example,probabilities,features])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
