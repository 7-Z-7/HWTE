...
"""
#with out DenseEinsum

import math
import six
import numpy as np
import tensorflow as tf
# import performer_util as util
...
class Attention(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    # attention_initializer = _glorot_initializer(input_shape.as_list()[-1],
    #                                             self.hidden_size)
    self.query_dense_layer = tf.keras.layers.Dense(
        units=self.hidden_size,
        use_bias=False,
        name="query")
    self.key_dense_layer = tf.keras.layers.Dense(
        units=self.hidden_size,
        use_bias=False,
        name="key")
    self.value_dense_layer = tf.keras.layers.Dense(
        units=self.hidden_size,
        use_bias=False,
        name="value")
  ...
  def call(self,
    ...
    input_shape=get_shape_list(query_input,expected_rank=3)

    query = self.query_dense_layer(query_input)
    key = self.key_dense_layer(source_input)
    value = self.value_dense_layer(source_input)

    query=tf.reshape(query,[input_shape[0], input_shape[1], self.num_heads, self.size_per_head])
    key=tf.reshape(key,[input_shape[0], input_shape[1], self.num_heads, self.size_per_head])
    value=tf.reshape(value,[input_shape[0], input_shape[1], self.num_heads, self.size_per_head])
    ...
    attention_output = favor_attention(query, key, value,
                                       ...
                                       projection_matrix)
    attention_output=tf.reshape(attention_output,[input_shape[0], input_shape[1],self.num_heads*self.size_per_head])
